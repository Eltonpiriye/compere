...
export default function OfflineGamePrompt({ idleMinutes = 10 }: Props) {
	...
	useEffect(() => {
		...
		if (!navigator.onLine) {
			setIsOffline(true); // Display the prompt immediately if already offline
			setShowGame(false);
			resetGame();
			setGameActive(true);
		}
	...
	}, [gameActive, gameScore, highScore]);
			...
			<Dialog open={isOffline}>
				<DialogContent className='sm:max-w-md max-w-[90vw] mx-auto'>
					...
					<p className='py-4 text-sm sm:text-base'>
						You've been idle for a while. Would you like to play our
						mini-game?
					</p>
