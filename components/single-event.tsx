...
export default function SingleEventPageFromCms({
  ...
  const textOpacity = useTransform(textScrollProgress, [0, 0.2, 0.8, 1], [1, 1, 0.7, 0.5])
  ...
  const handlePlay = () => {
    ...
    setTimeout(() => {
      if (videoRef.current) {
        videoRef.current.play().catch((e) => console.error("Main video play failed:", e))
      }
      if (backgroundVideoRef.current) {
        backgroundVideoRef.current.play().catch((e) => console.error("Background video play failed:", e))
      }
    }, 0)
  }
      ...
      <MaxWidthWrapper>
        <div className="relative" ref={mainRef}>
          ...
          <div className=" overflow-clip sticky top-20 " ref={textRef}>
            ...
            <p className="mt-5 md:mt-10 px-4 md:px-12 pb-8 text-xl md:text-[32px] font-bold ">{eventDescription}</p>
          </div>
          {/* Description Text */}
          <motion.div
          ...
          <div ref={galleryRef} className="relative z-50">
            <ImageGallery images={eventImages} scrollProgress={galleryScrollProgress} />
          </div>
        </div>
      </MaxWidthWrapper>
      {/* Final Marquee */}
